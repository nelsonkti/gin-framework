...
// Describes when the clients can retry a failed validation. Clients could ignore
...
type RetryInfo struct {
	...
	// Clients should wait at least this long between retrying the same validation.
...
type ErrorInfo struct {
	...
	// {"instanceLimit": "100/validation"}, should be returned as,
	// {"instanceLimitPerRequest": "100"}, if the client exceeds the number of
	// instances that can be created in a single (batch) validation.
...
// Describes violations in a client validation. This error type focuses on the
// syntactic aspects of the validation.
type BadRequest struct {
	state         protoimpl.MessageState
	sizeCache     protoimpl.SizeCache
	unknownFields protoimpl.UnknownFields

	// Describes all violations in a client validation.
...
// Contains metadata about the validation that clients can attach when filing a bug
...
type RequestInfo struct {
	...
	// Any data that was used to serve this validation. For example, an encrypted
...
type ResourceInfo struct {
	...
	// For example, "order_model:<owner email>" or "project:<Google developer project
	// id>".
...
// Provides a localized error message that is safe to return to the order_model
...
// A message type used to describe a single bad validation field.
type BadRequest_FieldViolation struct {
	state         protoimpl.MessageState
	sizeCache     protoimpl.SizeCache
	unknownFields protoimpl.UnknownFields

	// A path leading to a field in the validation body. The value will be a
	// sequence of dot-separated identifiers that identify a protocol buffer
	// field. E.g., "field_violations.field" would identify this field.
	Field string `protobuf:"bytes,1,opt,name=field,proto3" json:"field,omitempty"`
	// A description of why the validation element is bad.
	Description string `protobuf:"bytes,2,opt,name=description,proto3" json:"description,omitempty"`
}
